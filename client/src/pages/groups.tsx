...
import { Plus, Users, Crown, Clock, Search, ImagePlus, X } from "lucide-react";
...
export default function Groups() {
  ...
  // My groups (ones I created or am a member of)
  const myGroups = chatGroups.filter(g => g.createdById === user?.id);
  const myGroupIds = new Set(myGroups.map(g => g.id));

  // Public groups excluding ones I already own
  const publicGroups = chatGroups
    .filter(g => !g.isPrivate && !myGroupIds.has(g.id))
            ...
            <Card>
              <CardContent className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  {searchQuery ? "Try a different search term" : "Use the 'Create Group' button above to get started."}
                </p>
              </CardContent>
            </Card>
          ) : (
